		...
		json.data.forEach(setting => {
			...
			if (!setting.possible) {
				...
				if (multiselect.includes(setting.key)) {
					...
					temp += '<select class="setting" id="' + setting.key + '" name="' + setting.key + '" onchange="addRole(' + setting.key + ', ' + possible + ', this)"><option>Rolle hinzufügen...</option>';
					...
					setting.value.split(",").forEach(r => {
						temp += "<br><div><p>" + possible[r.split(":")[0]].name + '</p><input class="setting" value="' + r.split(":")[1] + '"><ion-icon name="close-outline" onclick="removeRole(' + setting.key + ', ' + possible + ', this, \'' + r.split(":")[0] + '\')"></ion-icon></div>';
					});
					temp += "</div>";
				} else {
