			...
			if (!setting.possible) {
				...
				if (multiselect.includes(setting.key)) {
					...
					setting.value.split(",").forEach(r => {
						temp += "<br><div><p>" + possible[r.split(":")[0]].name + '</p><input id="an_' + r.split(":")[0] + 'value" value="' + r.split(":")[1] + '"><ion-icon name="close-outline" onclick="removeRole(\'' + setting.key + '\', this, \'' + r.split(":")[0] + '\')"></ion-icon></div>';
					});
					temp += "</div>";
				} else {
