		...
		json.data.forEach(setting => {
			...
			if (setting.possible || typeof setting.value == "object") {
				...
				if (typeof setting.type == "string" && Array.isArray(setting.value) && (setting.type == "role" || setting.type.endsWith("channel") || multiselect.includes(setting.key)))
					...
					if (setting.value.length > 0 && typeof setting.value[0] != "object") temp += setting.value.map(i => addItem(setting, possible, i)).join("");
					...
					Object.keys(possible).forEach(key => {
						if (setting.type == "bool") console.log(typeof possible[key] == "boolean", possible[key], key, setting.value, setting.value ? " selected" : "")
						if (setting.type == "bool") temp += "<option value='" + key.replace("_", "") + "'" + (setting.value ? " selected" : "") + ">" + possible[key] + "</option>"
