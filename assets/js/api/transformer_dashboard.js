		...
		json.data.forEach(setting => {
			...
			if (setting.possible || typeof setting.value == "object") {
				...
				if (typeof setting.value == "object") {
					...
					else if (Object.keys(setting.value).length > 0) temp += addItem(setting, possible, setting.key, setting.value)//Object.keys(setting.value).map(i => addItem(setting, possible, i, setting.value[i])).join("");
					temp += "</div>";
				} else {
