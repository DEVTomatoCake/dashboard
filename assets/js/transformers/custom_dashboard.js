		...
		return "<h1>Your custom bots</h1>" +
			...
			bots.map(bot =>
				...
				(bot.hasAccess ?
					...
					"<br>" +
					"<button type='button' class='createForm green' id='startbutton-" + encode(bot.id) + "' onclick='startBot(\"" + encode(bot.id) + "\")'>" +
					"<ion-icon name='caret-up-outline'></ion-icon>Start/Restart</button>" +
