const formatVote = site =>
	"<li><a href='" + encode(site.url) + "' target='_blank' rel='noopener'>" + encode(site.name) + "</a>" +
...
document.addEventListener("DOMContentLoaded", async () => {
	if (getCookie("token")) {
		...
		if (json.status == "success") {
			document.getElementById("vote-tk").innerHTML = "<p>In total, you've voted <b>" + assertInt(json.totalVotes).toLocaleString() + "</b> times for TomatenKuchen.<br>" +
				"You currently have <b>" + assertInt(json.credits).toLocaleString() + " credits</b>.</p><br><h3 translation='user.votelinks'></h3>" +
